...
def doTraining(iteration_file, network_file):
    env = UnityEnvironment(file_name="D:\Banana_Windows_x86_64\Banana.exe",no_graphics=False)
...
#doTraining('iteration.pickle','network_3layer.pth')
scores_window={}
i=0
with open('iteration.pickle', 'rb') as handle:
    [scores_window, i] = pickle.load(handle)
fig = plt.figure()
ax = fig.add_subplot(111)
plt.plot(np.arange(1, len(scores_window)+1), i)
plt.ylabel('Score')
plt.xlabel('Episode #')
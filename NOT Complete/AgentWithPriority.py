...
import numpy as np
from ReplayBufferWithPriority import ReplayBufferWithPriority
import os
...
class AgentWithPriority:
    def __init__(self, env, action_size, state_size, use_dueling=False,
        ...
        self.memory = ReplayBufferWithPriority(self.seed, batch_size=BATCH_SIZE, device=self.device)
    ...
    def calcTDError(self,states, next_states,rewards,actions,dones):
        q_target = None
    ...
    def step(self, timestep, state, next_state, action, reward, done,probability):
        self.memory.add(state, next_state, action, reward, done,probability)
        if timestep % 4 == 0:
            if len(self.memory) >= BATCH_SIZE:
                states, next_states, actions, rewards, dones,probabilities = self.memory.sample()

                loss = self.calcTDError(states,next_state,rewards,actions,dones,probabilities)
